...
function createTab(){
  ...
  $("#tabs ul").append("<li><a href='#tab-" + cnt + "'>" + cnt + ": (" + x_low + ")-(" + x_high + ") (" + y_low + ")-(" + y_high + ")</li>");

  //add table to tab body
  $("#tabs").append("<div id='tab-" + cnt + "'>" + addTable() + "</div>");
...
function addTable(){
  ...
  $('#Tab').html(" ");
  //get values
  let x_high = Number($('#x_high').val());
  let x_low = Number($('#x_low').val());
  let y_high = Number($('#y_high').val());
  let y_low = Number($('#y_low').val()); 
